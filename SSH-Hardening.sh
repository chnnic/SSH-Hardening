...
# ============================================================
#  VPS 开荒脚本 V3.12.9 — 银趴火山帮
#  功能：SSH管理 / Fail2ban / BBR TCP 调优
#  V3.12.9: TCP 增强显示中文开关状态、推荐值和操作目标值，不改现有参数
#  V3.12.8: 修复离线安装同版本更新误报，按运行版本数字比较并过滤过期缓存
#  V3.12.7: 修复 BBR 转发/RA 顺序、接口回滚、死锁、旧参数迁移和预设并发覆盖
#  V3.12.6: TCP 增强独立开关、逐项验证与事务回滚、队列/配置/重传诊断
#  V3.12.5: 一键 DD / 系统重装模块增加中英文双语风险提示与安装阶段说明
#  V3.12.4: 修复 systemd-resolved 与 resolvconf 后端误判导致 DNS 未覆盖
#  V3.12.3: DNS 优化覆盖 openresolv 的接口/DHCP DNS，并在重启后保持
