...
# 打印一行带左右边框的内容，自动补空格对齐右边框
# 用法: box_line "内容（纯文本，不含转义色码）" "带色码的内容"
# 如果只传一个参数，则纯文本=带色码=同一内容
box_line() {
    local PLAIN="$1"   # 用于计算宽度的纯文本
    local COLORED="$2" # 实际显示内容（含色码）
    [ -z "$COLORED" ] && COLORED="$PLAIN"

    # 计算纯文本可见宽度（中文按2，ASCII按1）
    local VIS_LEN=0
    local i char
    local STR="$PLAIN"
    while [ -n "$STR" ]; do
        char="${STR:0:1}"
        STR="${STR:1}"
        # 判断是否为多字节中文（简单判断：ord > 127）
        if printf '%s' "$char" | grep -qP '[^\x00-\x7F]' 2>/dev/null; then
            VIS_LEN=$((VIS_LEN + 2))
        else
            VIS_LEN=$((VIS_LEN + 1))
        fi
    ...
    local PAD=$((BOX_W - 2 - VIS_LEN))  # 2 = 左边框"║ "
    [ "$PAD" -lt 0 ] && PAD=0
    local SPACES
    SPACES=$(printf '%*s' "$PAD" '')

    echo -e "${BOLD}${CYAN}║${NC}  ${COLORED}${SPACES}${BOLD}${CYAN}║${NC}"
}

box_sep() {
    echo -e "${BOLD}${CYAN}╠$(printf '═%.0s' $(seq 1 $((BOX_W-2))))╣${NC}"
}
...
box_title() {
    local TEXT="$1"
    # 计算居中
    local LEN=0 char STR="$TEXT"
    while [ -n "$STR" ]; do
        char="${STR:0:1}"; STR="${STR:1}"
        printf '%s' "$char" | grep -qP '[^\x00-\x7F]' 2>/dev/null && LEN=$((LEN+2)) || LEN=$((LEN+1))
    done
    local PAD_TOTAL=$(( BOX_W - 2 - LEN ))
    local PAD_L=$(( PAD_TOTAL / 2 ))
    local PAD_R=$(( PAD_TOTAL - PAD_L ))
    local SL SR
    SL=$(printf '%*s' "$PAD_L" '')
    SR=$(printf '%*s' "$PAD_R" '')
    echo -e "${BOLD}${CYAN}║${NC}${SL}${BOLD}${CYAN}${TEXT}${NC}${SR}${BOLD}${CYAN}║${NC}"
}

box_empty() {
    local SPACES
    SPACES=$(printf '%*s' "$((BOX_W-2))" '')
    echo -e "${BOLD}${CYAN}║${NC}${SPACES}${BOLD}${CYAN}║${NC}"
}
...
generate_key() {
    ...
    echo -e "  ${BOLD}${GREEN}┌─── 公钥（添加到服务器 authorized_keys）┐${NC}"
    echo ""
    echo "$PUBKEY"
    echo ""
