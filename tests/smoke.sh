...
for fn in systemd_available show_cli_help main_menu ssh_tools_menu ssh_key_count fail2ban_menu bbr_menu firewall_menu dns_menu dns_expected_nameservers dns_resolv_nameservers_match dns_resolvconf_configure dns_resolvconf_apply timesync_menu \
...
DNS_RESOLVCONF_CONFIG="$TMP/resolvconf.conf"
printf '%s\n' 'interface_order="lo"' '# BEGIN VPS TOOLS DNS' 'name_servers="old.example"' '# END VPS TOOLS DNS' > "$DNS_RESOLVCONF_CONFIG"
dns_resolvconf_configure '1.1.1.1 1.0.0.1' '2606:4700:4700::1111' true || { echo "openresolv configuration write failed" >&2; exit 1; }
dns_resolvconf_configure '8.8.8.8 8.8.4.4' '2001:4860:4860::8888' true || { echo "openresolv configuration replacement failed" >&2; exit 1; }
grep -qx 'name_servers="8.8.8.8 8.8.4.4 2001:4860:4860::8888"' "$DNS_RESOLVCONF_CONFIG" \
    || { echo "openresolv did not replace the managed DNS list" >&2; exit 1; }
grep -Fqx 'replace="${replace:-} nameserver/*/"' "$DNS_RESOLVCONF_CONFIG" \
    || { echo "openresolv interface DNS replacement rule is missing" >&2; exit 1; }
[ "$(grep -c '^# BEGIN VPS TOOLS DNS$' "$DNS_RESOLVCONF_CONFIG")" -eq 1 ] \
    || { echo "openresolv managed DNS block was duplicated" >&2; exit 1; }
DNS_RESOLV_SAMPLE="$TMP/resolv.conf"
printf '%s\n' '# Generated by resolvconf' 'nameserver 8.8.8.8' 'nameserver 8.8.4.4' > "$DNS_RESOLV_SAMPLE"
dns_resolv_nameservers_match "$DNS_RESOLV_SAMPLE" '8.8.8.8 8.8.4.4' '' false \
    || { echo "DNS nameserver result verification failed" >&2; exit 1; }
! dns_resolv_nameservers_match "$DNS_RESOLV_SAMPLE" '1.1.1.1' '' false \
    || { echo "DNS nameserver result verification accepted stale data" >&2; exit 1; }
