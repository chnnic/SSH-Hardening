...
grep -Fqx 'resolv_conf_passthrough=/dev/null' "$DNS_RESOLVCONF_CONFIG" \
    || { echo "openresolv interface DNS override is missing" >&2; exit 1; }
[ "$(grep -c '^# BEGIN VPS TOOLS DNS$' "$DNS_RESOLVCONF_CONFIG")" -eq 1 ] \
    || { echo "openresolv managed DNS block was duplicated" >&2; exit 1; }
(
    DNS_RESOLV_LINK="$TMP/systemd-resolv.conf"
    DNS_RESOLVECTL_BIN="$TMP/bin/resolvectl"
    mkdir -p "$TMP/bin"
    : > "$DNS_RESOLVECTL_BIN"
    chmod 755 "$DNS_RESOLVECTL_BIN"
    svc_is_active() { [ "$1" = systemd-resolved ]; }
    PATH="$TMP/bin:$PATH"
