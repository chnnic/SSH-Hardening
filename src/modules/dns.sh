...
dns_show_current() {
    ...
    awk '!seen[$2]++ && /^nameserver[[:space:]]+/ { print }' /etc/resolv.conf 2>/dev/null | while read -r line; do
...
dns_systemd_resolved_linked() {
    local RESOLV="$1" RESOLVECTL_BIN="${DNS_RESOLVECTL_BIN:-resolvectl}" TARGET
    command -v "$RESOLVECTL_BIN" >/dev/null 2>&1 || return 1
    svc_is_active systemd-resolved || return 1
    [ -L "$RESOLV" ] || return 1
    TARGET=$(readlink -f "$RESOLV" 2>/dev/null || true)
    [ -n "$TARGET" ] || TARGET=$(readlink "$RESOLV" 2>/dev/null || true)
    case "$TARGET" in
        */run/systemd/resolve/*) return 0 ;;
        *) return 1 ;;
    esac
}

dns_systemd_resolved_configure() {
    local ALL_DNS="$1"
    local CONF="${DNS_RESOLVED_CONFIG:-/etc/systemd/resolved.conf.d/99-vps-tools.conf}" TMP
...
dns_systemd_resolved_link_apply() {
    local IFACE="$1" ALL_DNS="$2"
    local RESOLVECTL_BIN="${DNS_RESOLVECTL_BIN:-resolvectl}"
    command -v "$RESOLVECTL_BIN" >/dev/null 2>&1 || return 1
    "$RESOLVECTL_BIN" dns "$IFACE" $ALL_DNS || return 1
    "$RESOLVECTL_BIN" domain "$IFACE" '~.' || return 1
}

dns_systemd_resolved_link_match() {
    local IFACE="$1" ALL_DNS="$2" ACTUAL EXPECTED
    local RESOLVECTL_BIN="${DNS_RESOLVECTL_BIN:-resolvectl}"
    command -v "$RESOLVECTL_BIN" >/dev/null 2>&1 || return 1
    ACTUAL=$("$RESOLVECTL_BIN" dns "$IFACE" 2>/dev/null | awk -F': ' 'NF > 1 { print $2; exit }')
    EXPECTED="$ALL_DNS"
    [ "$ACTUAL" = "$EXPECTED" ]
}

dns_networkmanager_apply() {
    local V4_LIST="$1" V6_LIST="$2" HAS_V6="$3" CON NM_COUNT=0
...
dns_resolvconf_configure() {
    ...
    fi
    {
        [ ! -s "$TMP" ] || printf '\n'
        printf '%s\n' '# BEGIN VPS TOOLS DNS'
        # /dev/null makes openresolv ignore interface/DHCP DNS sources.
...
dns_write() {
    ...
    if command -v nmcli >/dev/null 2>&1 && svc_is_active NetworkManager; then
        ...
        dns_networkmanager_apply "$V4_LIST" "$V6_LIST" "$HAS_V6" || return 1
    elif dns_systemd_resolved_linked "$RESOLV"; then
        BACKEND="systemd-resolved"
        dns_systemd_resolved_configure "$ALL_DNS" \
            || { error "无法写入 systemd-resolved 配置"; return 1; }
        svc_restart systemd-resolved || { error "systemd-resolved 重启失败"; return 1; }
        local RESOLVED_IFACE; RESOLVED_IFACE=$(default_iface)
        if [ -n "$RESOLVED_IFACE" ]; then
            dns_systemd_resolved_link_apply "$RESOLVED_IFACE" "$ALL_DNS" \
                || { error "systemd-resolved 链路 DNS 写入失败：$RESOLVED_IFACE"; return 1; }
            dns_systemd_resolved_link_match "$RESOLVED_IFACE" "$ALL_DNS" \
                || { error "systemd-resolved 链路 DNS 校验失败：$RESOLVED_IFACE"; return 1; }
        else
            error "无法确定默认网卡，未修改 systemd-resolved 链路 DNS"
            return 1
        fi
    ...
    echo ""
    echo -e "  ${BOLD}更新后：${NC}"
    awk '!seen[$2]++ && /^nameserver[[:space:]]+/ { print }' "$RESOLV" | while read -r line; do
        local IP; IP=$(echo "$line" | awk '{print $2}')
